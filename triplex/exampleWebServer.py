...
import random
import numpy as np
import algorithmStocks3 as algorithmStocks
#Global variables
#########################################
app = bottle.Bottle()
...
def sectorBezier(stocks_id):
	...
	for i in range(len(stocks)):
		...
		for j in range(len(stocks[i])):
			...
			#xAxis.append(str(((thisTime.hour*60+thisTime.minute-60*9)/3)-minT))
			if j!=0:
				xAxis.append((float(j)/float(rangeT)))
			else:
				xAxis.append((0.0))
	...
	for i in range(len(processedData)):
		for j in range(len(processedData[i]['z'])):
			processedData[i]['z'][j] = ((float(processedData[i]['z'][j])-minZProcessed)*rangeZ)
		processedData[i]['z'].append(processedData[i]['z'][-1])
	#end normalizartion
	...
	for i in range(len(processedData)):
		...
		processedData[i]['z'] = curve.result
	#end bezier

	for i in range(len(processedData)):
		finalAxis.append({})
		finalAxis[i]['z'] = [ str(n[1]) for n in processedData[i]['z'] ]
		finalAxis[i]['y'] = [str(k) for k in getY(zip(processedData[i]['x'],processedData[i]['y']), [ n[0] for n in processedData[i]['z'] ])]
		finalAxis[i]['x'] = [ str(n[0]) for n in processedData[i]['z']]

	return '^'.join([','.join(stock['x'])+"\n"+','.join(stock['y'])+"\n"+','.join(stock['z']) for stock in finalAxis])
#Retrieves the stocks xyz axis using a polynomial fit
@app.route('/stocks-xyz2/<stocks_id>')

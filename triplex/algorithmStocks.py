...
def sectorBezier(stocks_id):
	...
	for stock in stocksList:
		stocks.append(ys.historical(stock, {'month':1, 'day': 1, 'year':2014},{'month':1, 'day': 10, 'year':2014}))
	...
	minZProcessedR = 10000.0
	maxZProcessedR = -10000.0
	minZProcessed = 10000.0
	maxZProcessed = -10000.0
	#---- finds the limits for the z axis normalization
	#---- those values locate and eliminate the crazy extrema
	jMax = 0
	jMin = 0
	iMax = 0
	iMin = 0
	for i in range(stocksLength):
		for j in range(axisLimit-1):
			if float(processedData[i][2][j]) < minZProcessedR:
				minZProcessedR = float(processedData[i][2][j])
				jMin = j
				iMin = i
			if float(processedData[i][2][j]) > maxZProcessedR:
				maxZProcessedR = float(processedData[i][2][j])
				jMax = j
				iMax = i
	processedData[iMax][2][jMax] = processedData[iMax][2][-1]
	processedData[iMin][2][jMin] = processedData[iMin][2][-1]
	#---- now that we regulated the crazy extrema, let's find a better normalization
	for i in range(stocksLength):
		for j in range(axisLimit-1):
	...
	for i in range(stocksLength):
		...
		processedData[i][2].append(processedData[i][2][-1])
	print printList(processedData)
	finalAxis = processedData
	#---- create new values using bezier curve
	# finalAxis = []
	# for i in range(len(processedData)):
	# 	curve = bezier.Curve()
	# 	curve.draw(zip(processedData[i][0],processedData[i][2]), 200)

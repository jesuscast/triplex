...
class Curve:
    '''
    Implementation of de Casteljau's algorithm for drawing Bezier curves.
    ...
    def add_point(self, x0, y0):
        self.result.append((x0,y0))
    ...
    def draw_curve(self, points, t):
        ...
        if (len(points) == 1):
            self.add_point(points[0][0], points[0][1])
        else:
            newpoints = []
    ...
    # that recursion comes with. Only quadratical Bezier curves.
    ...
    def draw(self, points, totalPoints):
        t = 0
        step = 1.0/totalPoints
        while (t <= 1):
...
#     # Finally draw some Bezier curves :)
